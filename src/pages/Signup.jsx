...
const Signup = () => {
  ...
  useEffect(() => {
    if (!widgetScriptLoaded.current) {
      ...
      script.async = true;
      script.onload = () => { widgetScriptLoaded.current = true; };
      document.body.appendChild(script);
    }
  }, []);

  useEffect(() => {
    if (phone.match(/^\d{10}$/) && !otpVerified && !otpWidgetShown && widgetScriptLoaded.current && !phoneImmediateError) {
      triggerOtpWidget();
  ...
  const handleSubmit = async (e) => {
    ...
    if (!phone.match(/^\d{10}$/)) { setError('Please enter your 10-digit phone number'); setIsLoading(false); return; }
    if (!otpVerified) { setError('Please verify your phone number with OTP.'); setIsLoading(false); return; }

    try {
      const response = await fetch(`${config.API_URLS.AUTH}/register`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Accept: 'application/json' },
        body: JSON.stringify({ name: formData.name, email: formData.email, password: formData.password, phone: '91' + phone })
      });
      const data = await response.json();
      if (response.ok) { toast.success('Account created successfully!'); navigate('/login'); } 
  ...
  const triggerOtpWidget = () => {
    setOtpError(""); setOtpSuccess(""); setOtpLoading(true);
    if (!window.initSendOTP) { setOtpError("OTP widget not loaded."); setOtpLoading(false); return; }
    window.initSendOTP({
      widgetId: "3567686d316c363335313136",
      tokenAuth: "458779TNIVxOl3qDwI6866bc33P1",
      identifier: '91' + phone,
      success: () => { setOtpVerified(true); setOtpSuccess("Phone verified!"); setOtpLoading(false); },
      failure: (error) => { setOtpError(error?.message || "OTP failed"); setOtpVerified(false); setOtpLoading(false); },
    });
  };
    ...
    <div className="min-h-screen flex items-center justify-center relative font-sans bg-gradient-to-b from-blue-300 via-blue-400 to-blue-600 overflow-hidden">
      ...
      >
        <div className="text-center mb-6">
          <h2 className="text-4xl font-bold text-blue-100 mb-2 tracking-tight">Create <span className="italic text-white font-serif">Account</span></h2>
          <p className="text-sm text-blue-50">Already have an account? <Link to="/login" className="font-semibold text-blue-300 hover:underline">Sign in</Link></p>
        </div>
        ...
        {(error || otpError || otpSuccess) && (
          <div className="space-y-2 mb-4 text-center">
            ...
            {otpSuccess && <div className="text-green-300 text-sm">{otpSuccess}</div>}
          </div>
        )}
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div>
            ...
            <div className="mt-1 relative flex">
              ...
              <input
                id="phone" name="phone" type="tel" required pattern="[0-9]{10}" maxLength={11}
                ...
                value={phone}
                onChange={e => { let val = e.target.value.replace(/\D/g, '').slice(0, 11); setPhone(val); setPhoneImmediateError(val.length === 11 ? 'Please enter a 10-digit phone number' : ''); }}
                disabled={isLoading || otpVerified}
              />
            </div>
            {phoneImmediateError && <div className="text-red-400 text-sm mt-1">{phoneImmediateError}</div>}
          </div>
          ...
          >
            {isLoading || otpLoading && <span className="animate-spin h-5 w-5 border-2 border-white border-t-transparent rounded-full"/>}
            <span>{isLoading || otpLoading ? 'Processing...' : 'Create Account'}</span>
          </button>
        </form>

        {/* Fun water SVG waves at bottom */}
...
const InputField = ({ id, label, icon, placeholder, value, onChange, disabled }) => (
  <div>
    ...
    <div className="mt-1 relative">
      ...
      <input
        id={id} name={id} type="text" autoComplete={id} required
  ...
  <div>
    ...
    <div className="mt-1 relative">
      ...
      <input
        ...
        placeholder={label} value={value} onChange={onChange} disabled={disabled}
      />
